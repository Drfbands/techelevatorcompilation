<template>
    <div class="todo-list">
        ...
        <ul>
            <li v-for="todo in todos" :key="todo.id" :class="{'todo-completed': todo.completed}">
...
export default {
    ...
    methods: {
    changeStatus(id) {
    const indexOfCorrectTodo = this.todos.findIndex((todo) => todo.id === id);
    this.todos[indexOfCorrectTodo].completed = !this.todos[indexOfCorrectTodo].completed;
    }
}
}
</script>
